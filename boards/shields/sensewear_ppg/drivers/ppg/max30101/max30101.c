...
} max30101 = {.device = I2C_DT_SPEC_GET(MAX30101_NODE)};
...
static void ppg_batch_flush_channel(struct ppg_channel_batch_t* batch,
									int (*notify_fn)(const struct ppg_sample_notification_t*,
													 size_t)) {
	if (batch->count == 0U) {
		return;
	}
...
__STATIC_INLINE void max30101_i2c_write_registers(enum max30101_register_type startAddress,
												  uint8_t* value,
												  size_t count) {
...
__STATIC_INLINE void max30101_i2c_read_registers(enum max30101_register_type startAddress,
												 uint8_t* value,
												 size_t count) {
...
static void sensor_data_work_handler() {
	...
	size_t actualSize =
		max30101_read_fifo(buffer, MAX_SENSOR_READING_SIZE * max30101.led_count * 3);
	...
	if ((red_batch.count >= PPG_BATCH_TARGET_SAMPLES && red_batch.count >= PPG_BATCH_MIN_SAMPLES) ||
		...
		(green_batch.count >= PPG_BATCH_TARGET_SAMPLES &&
		 green_batch.count >= PPG_BATCH_MIN_SAMPLES) ||
		red_batch.count >= PPG_BATCH_MAX_SAMPLES || ir_batch.count >= PPG_BATCH_MAX_SAMPLES ||
...
void set_ppg_operation_mode(uint16_t operation_mode) {
	if (operation_mode != max30101_mode_MultiLed && operation_mode != max30101_mode_HeartRate &&
		operation_mode != max30101_mode_SpO2) {
...
void ppg_set_streaming_enabled(bool enabled) {
	if (enabled) {
		...
		if (!ppg_thread_started) {
			...
			k_thread_create(&ppg_sensor_thread,
							ppg_sensor_stack,
							K_THREAD_STACK_SIZEOF(ppg_sensor_stack),
							ppg_sensor_thread_fn,
							NULL,
							NULL,
							NULL,
							K_PRIO_PREEMPT(5),
...
void max30101_config(void) {
	...
	/* Reset the sensor */
	{
		...
		max30101_i2c_write_registers(max30101_register_ModeConfiguration, (void*) &value, 1);

		/* Wait for reset to be cleared */
		do {
			max30101_i2c_read_registers(max30101_register_ModeConfiguration, (void*) &value, 1);
	...
	/* Write the FIFO configuration register */
	{
		...
		max30101_i2c_write_registers(max30101_register_FIFO_Configuration, (void*) &value, 1);
	}
	...
	/* Write the SpO2 configuration register */
	{
		...
		max30101_i2c_write_registers(max30101_register_SpO2Configuration, (void*) &value, 1);

		max30101_i2c_read_registers(max30101_register_SpO2Configuration, (void*) &value, 1);
	...
	/* Write the LED pulse amplitude registers */
	{
		...
		max30101_i2c_write_registers(max30101_register_ProxModeLED_PA, &proximityAmplitude, 1);
	}
	/* Write the Multi-LED mode register */
	{
		union max30101_multi_led_mode_control_t* multiLedModeConfig = (void*) &value;
		...
		max30101_i2c_write_registers(max30101_register_ModeControlReg1, (void*) &value, 2);
	}
	...
	/* Read the configuration registers to calculate sampling rate */
	{
		...
		max30101_i2c_read_registers(max30101_register_FIFO_Configuration, (void*) &value, 1);
	...
	{
		value = 0;
		max30101_i2c_write_registers(max30101_register_FIFO_WritePointer, (void*) &value, 3);
	}
	...
	/* Write Interrupt Enable registers to zero */
	{
		...
		intEnable->value = 0;
		max30101_i2c_write_registers(max30101_register_InterruptEnable1, (void*) &value, 2);
	}
...
size_t max30101_read_fifo(void* buffer, size_t bufferSize) {
	...
	max30101_i2c_read_registers(max30101_register_FIFO_WritePointer, value, 3);
	...
	byteCount = (size_t) availableSamples * MAX30101_BYTES_PER_CHANNEL * max30101.led_count;
	...
	max30101_i2c_read_registers(max30101_register_FIFO_DataRegister, buffer, byteCount);
...
void max30101_irq_handler(void) {
	...
	max30101_i2c_read_registers(max30101_register_InterruptStatus1, (void*) &value, 2);
	...
	if (intStatus->bits.ppg_rdy != 0) {
		...
		if (max30101.state.bits.bDetectingProximity != 0) {
			...
			unsigned int value = ((buffer[0] << 16) + (buffer[1] << 8) + buffer[2]) & 0x03FFFFF;
			printf("PPG %u IR 0x%08x\r\n", ret, value);
			max30101.proximity_led_value_sum += value;
			max30101.proximity_led_read_count++;

			if (max30101.proximity_led_read_count >= MAX30101_PROXIMITY_READCOUNT_FOR_DECISION) {
				uint32_t irLedValue =
					max30101.proximity_led_value_sum / max30101.proximity_led_read_count;
				irLedValue >>= 10;
