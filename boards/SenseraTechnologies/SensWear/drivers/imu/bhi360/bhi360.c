...
BUILD_ASSERT(DT_NODE_HAS_PROP(BHI360_NODE, cs_gpios),
             "BHI360 is missing its chip-select GPIO");
...
static const struct sys_spi_dt_spec bhi360_spi =
    SYS_SPI_DT_SPEC_GET(BHI360_NODE, SPI_WORD_SET(8) | SPI_TRANSFER_MSB);
 ...
 */
static const struct gpio_dt_spec bhi360_cs = GPIO_DT_SPEC_GET(BHI360_NODE, cs_gpios);

/** Assert (select) the BHI360 chip-select line. */
static inline void bhi360_cs_select(void)
{
    gpio_pin_set_dt(&bhi360_cs, 1);
}

/** Deassert (release) the BHI360 chip-select line. */
static inline void bhi360_cs_deselect(void)
{
    gpio_pin_set_dt(&bhi360_cs, 0);
}
...
{
    if (!sys_spi_is_ready(&bhi360_spi)) {
        LOG_ERR("BHI360 SYS_SPI device not ready");
        return -ENODEV;
    }

    if (!gpio_is_ready_dt(&bhi360_cs)) {
        LOG_ERR("BHI360 chip-select GPIO not ready");
        return -ENODEV;
    }

    int ret = gpio_pin_configure_dt(&bhi360_cs, GPIO_OUTPUT_INACTIVE);
    if (ret < 0) {
        LOG_ERR("Failed to configure BHI360 chip-select GPIO");
        return ret;
    }
                              ...
                              void *intf_ptr)
{
    ...
    int ret = sys_spi_lock(&bhi360_spi, K_MSEC(100));
    if (ret == 0) {
        bhi360_cs_select();
        ret = sys_spi_transceive(&bhi360_spi, &tx, &rx);
        bhi360_cs_deselect();
        int unlock_ret = sys_spi_unlock(&bhi360_spi);
        if (ret == 0) {
            ret = unlock_ret;
        }
    }
                               ...
                               void *intf_ptr)
{
    ...
    if (ret == 0) {
        bhi360_cs_select();
        ret = sys_spi_write(&bhi360_spi, &tx);
        bhi360_cs_deselect();
        int unlock_ret = sys_spi_unlock(&bhi360_spi);
        if (ret == 0) {
            ret = unlock_ret;
